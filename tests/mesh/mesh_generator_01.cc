...
template <int dim>
void test (std::unique_ptr<MeshGenerator<dim>> &msh_ptr,
...
int main (int argc, char *argv[])
{
  ...
  // 2D test triangulation and mesh construction
  dealii::parallel::distributed::Triangulation<2> tria_2d (MPI_COMM_WORLD);
  std::unique_ptr<MeshGenerator<2>> msh_ptr =
      std::unique_ptr<MeshGenerator<2>> (new MeshGenerator<2>(prm));
  
  // testing
  test<2> (msh_ptr, tria_2d);
