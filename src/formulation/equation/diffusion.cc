...
void Diffusion<dim>::FillCellScatteringSource(
  ...
  SetCell(cell_ptr);
  MaterialID material_id = cell_ptr->material_id();
  int total_groups = scalar_flux_ptrs.size();
  ...
  for (int group_in = 0; group_in < total_groups; ++group_in) {
    if (group_in != group) {
      ...
      auto sigma_s = cross_sections_->sigma_s.at(material_id)(group_in, group);

      if (sigma_s > 1e-14) {

        finite_element_->values()->get_function_values(
            (*scalar_flux_ptrs.at(group_in)),
