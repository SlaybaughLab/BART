...
void FormulationUpdaterDiffusionTest<DimensionWrapper>::SetUp() {
  ...
  test_updater_ptr_ = std::make_unique<UpdaterType>(std::move(formulation_ptr), std::move(stamper_ptr),
...
TYPED_TEST(FormulationUpdaterDiffusionTest, Constructor) {
  ...
  EXPECT_NO_THROW({
        test_updater_ptr = std::make_unique<UpdaterType>(std::move(formulation_ptr), std::move(stamper_ptr));
      });
...
TYPED_TEST(FormulationUpdaterDiffusionTest, ConstructorBadDependencies) {
  ...
  auto stamper_ptr = std::make_unique<StamperType>();
  std::unique_ptr<UpdaterType> test_updater_ptr;
  EXPECT_ANY_THROW({ test_updater_ptr = std::make_unique<UpdaterType>(nullptr, std::move(stamper_ptr)); });
  EXPECT_ANY_THROW({ test_updater_ptr = std::make_unique<UpdaterType>(std::move(formulation_ptr), nullptr); });
  EXPECT_ANY_THROW({ test_updater_ptr = std::make_unique<UpdaterType>(nullptr, nullptr); });
}
    ...
    if (cell->at_boundary()) {
      ...
      for (int face = 0; face < faces_per_cell; ++face) {
        if (cell->face(face)->at_boundary()) {
          ...
          EXPECT_CALL(*this->formulation_obs_ptr_, FillBoundaryTerm(_, cell, face, boundary_type));
        }
      }
    }
  }
...
TYPED_TEST(FormulationUpdaterDiffusionTest, UpdateFixedTermWithRHSConstantTest) {
  system::EnergyGroup group_number(this->group_number);
  quadrature::QuadraturePointIndex angle_index(this->angle_index);
  bart::system::Index scalar_index{this->group_number, 0};
  ...
  EXPECT_CALL(*this->stamper_obs_ptr_, StampMatrix(Ref(*this->matrix_to_stamp), _))
      .Times(2).WillRepeatedly(DoDefault());
  EXPECT_CALL(*this->stamper_obs_ptr_, StampBoundaryMatrix(Ref(*this->matrix_to_stamp), _)).WillOnce(DoDefault());
  EXPECT_CALL(*this->stamper_obs_ptr_, StampVector(Ref(*this->vector_to_stamp), _))
      .Times(2).WillRepeatedly(DoDefault());
...
TYPED_TEST(FormulationUpdaterDiffusionTest, UpdateScatteringSourceTest) {
  ...
  EXPECT_CALL(*this->mock_rhs_obs_ptr_, GetVariableTermPtr(scalar_index,
                                                           system::terms::VariableLinearTerms::kScatteringSource))
      .WillOnce(DoDefault());
  EXPECT_CALL(*this->stamper_obs_ptr_, StampVector(_,_)).WillOnce(DoDefault());
  EXPECT_CALL(*this->current_moments_obs_ptr_, moments()).WillOnce(DoDefault());
  for (auto& cell : this->cells_) {
    EXPECT_CALL(*this->formulation_obs_ptr_, FillCellScatteringSource(_, cell, group_number.get(),
...
TYPED_TEST(FormulationUpdaterDiffusionTest, UpdateFissionSourceTest) {
  ...
  for (auto& cell : this->cells_) {
    EXPECT_CALL(*this->formulation_obs_ptr_,
                FillCellFissionSource(_, cell, group_number.get(), k_effective,
...
TYPED_TEST(FormulationUpdaterDiffusionTest, UpdateFixedSourceTest) {
  ...
  for (auto& cell : this->cells_) {
    EXPECT_CALL(*this->formulation_obs_ptr_,FillCellFixedSource(_, cell, group_number.get())).WillOnce(DoDefault());
  }
