...
class QuadraturePointTest : public ::testing::Test {
 public:
  ...
  std::shared_ptr<quadrature::OrdinateI<dim>> ordinate_ptr { std::make_shared<quadrature::OrdinateMock<dim>>() };
};
...
TYPED_TEST(QuadraturePointTest, ConstructorAndGetters) {
  ...
  quadrature::QuadraturePoint<dim> new_point(this->ordinate_ptr, quadrature::Weight(weight));
...
TYPED_TEST(QuadraturePointTest, CartesianPositionAndTensor) {
  ...
  EXPECT_CALL(*mock_ptr, cartesian_position()).WillOnce(::testing::Return(position));

  dealii::Tensor<1, dim> tensor_position;
  for (int i = 0; i < dim; ++i)
    tensor_position[i] = 1;

  EXPECT_CALL(*mock_ptr, cartesian_position_tensor()).WillOnce(::testing::Return(tensor_position));
