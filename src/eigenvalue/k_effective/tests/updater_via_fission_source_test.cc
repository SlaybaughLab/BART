...
TEST_F(EigKEffUpdaterViaFissionSourceTest, CalculateKEff) {
  ...
  EXPECT_DOUBLE_EQ(test_k_eff_updater.current_fission_source(), fission_sources[0]);
  EXPECT_DOUBLE_EQ(test_k_eff_updater.k_effective(), fission_sources[0]);
  ...
  EXPECT_DOUBLE_EQ(test_k_eff_updater.current_fission_source(), fission_sources[1]);
  EXPECT_DOUBLE_EQ(test_k_eff_updater.previous_fission_source(), fission_sources[0]);
  EXPECT_DOUBLE_EQ(test_k_eff_updater.k_effective(), k_effective);
  ...
  EXPECT_NEAR(k_effective, expected_result, 1e-12);
  EXPECT_DOUBLE_EQ(test_k_eff_updater.current_fission_source(), fission_sources[2]);
  EXPECT_DOUBLE_EQ(test_k_eff_updater.previous_fission_source(), fission_sources[1]);
  EXPECT_DOUBLE_EQ(test_k_eff_updater.k_effective(), k_effective);
