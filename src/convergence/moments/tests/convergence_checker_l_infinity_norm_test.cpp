...
class MomentConvergenceCheckerLInfinityNormTest : public ::testing::Test {
 public:
  using Vector = dealii::Vector<double>;
  using Checker = convergence::moments::ConvergenceCheckerLInfinityNorm;
  static constexpr double max_delta{ 1e-6 };
  static constexpr Vector::size_type moment_size{5 };
  Checker checker{ max_delta };
  Vector moment_one, moment_two;
  auto SetUp() -> void override;
};

auto MomentConvergenceCheckerLInfinityNormTest::SetUp() -> void {
...
TEST_F(MomentConvergenceCheckerLInfinityNormTest, Getters) {
  EXPECT_EQ(checker.max_delta(), this->max_delta);
}

// Max delta setter should set value correctly
TEST_F(MomentConvergenceCheckerLInfinityNormTest, MaxDeltaSetter) {
...
TEST_F(MomentConvergenceCheckerLInfinityNormTest, SetterBadValue) {
...
TEST_F(MomentConvergenceCheckerLInfinityNormTest, SameVector) {
...
TEST_F(MomentConvergenceCheckerLInfinityNormTest, OneThresholdAway) {
  double to_add = moment_one.linfty_norm() * 0.99 * checker.max_delta();
  moment_two(2) += to_add;
...
TEST_F(MomentConvergenceCheckerLInfinityNormTest, TwoThresholdAway) {
  const double to_add = moment_one.linfty_norm() * 2 * checker.max_delta();
  moment_two(2) += to_add;
...
TEST_F(MomentConvergenceCheckerLInfinityNormTest, SetMaxDelta) {
  const double to_set{ 1e-5 };
  checker.SetMaxDelta(to_set);
  EXPECT_EQ(checker.max_delta(), to_set);

  const double to_add = moment_one.linfty_norm() * 0.99 * to_set;
  moment_two(2) += to_add;
