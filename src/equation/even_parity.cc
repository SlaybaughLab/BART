...
void EvenParity<dim>::PreassembleCellMatrices () {
  ...
  for (int qi=0; qi<this->n_q_; ++qi)
    for (int dir=0; dir<this->n_dir; ++dir) {
      this->pre_streaming_[{dir, qi}] =
          dealii::FullMatrix<double> (this->dofs_per_cell_, this->dofs_per_cell_);
      for (int i=0; i<this->dofs_per_cell_; ++i)
        for (int j=0; j<this->dofs_per_cell_; ++j)
          this->pre_streaming_[{dir, qi}](i,j) =
             (this->fv_->shape_grad(i,qi) * this->omega_[i_dir])
             *
             (this->fv_->shape_grad(j,qi) * this->omega_[i_dir]);
    }
}
    ...
    for (int i=0; i<this->dofs_per_cell_; ++i)
      for (int j=0; j<this->dofs_per_cell_; ++j)
        cell_matrix(i,j) += (this->pre_streaming_[{dir, qi}](i,j) *
                             this->xsec_->inv_sigt[mid][g]
                             +
                             this->pre_collision_[qi](i,j) *
                             this->xsec_->sigt[mid][g]) * this->fv_->JxW(qi);
}
...
void EvenParity<dim>::IntegrateBoundaryBilinearForm (
  ...
  if (this->have_reflective_bc_ && this->is_reflective_bc_[bd_id]) {
    double inv_sigt =
        this->xsec_->inv_sigt[this->equ_name_][cell->material_id()][g];
    // hard coded part
    Tensor<1, dim> ref_angle =
    this->omega_i[i_dir] - 2.0 * (this->omega_i[i_dir] * vec_n) * vec_n;
    // standard part: not stable
    double ndo_inv_sigt = vec_n * this->omega_[i_dir] * inv_sigt;
    for (int qi=0; qi<this->n_q_f; ++qi)
      for (int i=0; i<this->dofs_per_cell_; ++i)
        for (int j=0; j<this->dofs_per_cell_; ++j)
...
void EvenParity<dim>::IntegrateInterfaceBilinearForm (
  ...
  double local_sigt = this->xsec_->sigt[mid][g];
  double local_inv_sigt = this->xsec_->inv_sigt[mid][g];
  double local_measure = cell->measure ();
  double neigh_sigt = this->xsec_->sigt[mid_nei][g];
  double neigh_inv_sigt = this->xsec_->inv_sigt[mid_nei][g];
  ...
  double sige = std::max(0.25, tensor_norms_[i_dir] * c_penalty_ * avg_mfp_inv);

  double half_ndo = 0.5 * vec_n * this->omega_[i_dir];
    ...
    for (int i=0; i<this->dofs_per_cell_; ++i)
      for (int j=0; j<this->dofs_per_cell_; ++j) {
        vp_up(i,j) += (sige *
            ...
            (this->omega_i[i_dir] * this->fvf_->shape_grad(i,qi)) *
            this->fvf_->shape_value(j,qi)
            -
            local_inv_sigt * half_ndo *
            this->fvf_->shape_value(i,qi) *
            (this->omega_i[i_dir] * this->fvf_->shape_grad(j,qi)))
            * this->fvf_->JxW(qi);
        ...
        vp_un(i,j) += (-sige *
            ...
            this->fvf_->shape_value(i,qi) *
            (this->omega_i[i_dir] * this->fvf_nei_->shape_grad(j,qi))
            ) * this->fvf_->JxW(qi);
        ...
        vn_up(i,j) += (-sige *
            ...
            -
            neigh_inv_sigt * half_ndo *
            (this->omega_i[i_dir] * this->fvf_nei_->shape_grad(i,qi)) *
            this->fvf_->shape_value(j,qi)
            +
            local_inv_sigt * half_ndo *
            this->fvf_nei_->shape_value(i,qi) *
...
void EvenParity<dim>::IntegrateScatteringLinearForm (
  ...
  for (int gin=0; gin<this->n_group_; ++gin) {
    ...
    for (int qi=0; qi<this->n_q_; ++qi)
      q_at_qp[qi] += (this->xsec_->sigs_per_ster[mid](gin,g) *
                      local_flx[qi]);
  }
...
void EvenParity<dim>::IntegrateCellFixedLinearForm (
  ...
  if (!this->is_eigen_problem_) {
    q_at_qp = std::vector<double> (n_q,
        this->xsec_->q_per_ster[mid][g]);
  } else if (this->is_eigen_problem_ && this->xsec_->is_material_fissile[mid]) {
    for (int gin=0; gin<this->n_group_; ++gin)
    {
      std::vector<double> local_flx (n_q);
      this->fv_->get_function_values (
          this->mat_vec_->moments[this->equ_name_][{gin, 0, 0}], local_flx);
      for (int qi=0; qi<n_q; ++qi)
        q_at_qp[qi] += this->scaled_fiss_transfer_per_ster_[mid](gin, g) *
            local_flx[qi];
    }
  }
  // calculate cell rhs:
