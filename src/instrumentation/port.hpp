#ifndef BART_SRC_INSTRUMENTATION_PORT_HPP_
#define BART_SRC_INSTRUMENTATION_PORT_HPP_
...
class Port {
 public:
  ...
  void AddInstrument(std::shared_ptr<InstrumentType> instrument_ptr) {instrument_ptr_ = instrument_ptr; }
...
PortType& GetPort(T& to_expose) {
  static_assert(std::is_polymorphic_v<T>,
      "Error in GetPort, passed object must be a reference to a polymorphic class that can be cast to a Port. Pointers "
      "cannot be passed, you must dereference to the underlying object using *");
  return dynamic_cast<PortType&>(to_expose);
}

} // namespace bart::instrumentation

#endif //BART_SRC_INSTRUMENTATION_PORT_HPP_
