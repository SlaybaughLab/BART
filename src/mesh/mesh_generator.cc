...
MeshGenerator<dim>::MeshGenerator (dealii::ParameterHandler &prm)
  ...
  if (!is_mesh_generated_) {
    ...
    if (is_mesh_unstructed_)
      pin_radius_ = prm.get ("pin radius");
  } else {
    ...
    if (cell->is_locally_owned()) {
      ...
      if (is_mesh_unstructed_) {
        ...
        if (fuel_id==0) {
          ...
          GetPinCenter(cell, pin_center);
          
          if (center.distance(pin_center)<pin_radius_) {
    ...
    if (in.is_open ()) {
      while (std::getline (in, line)) {
        ...
        for (unsigned int x=0; x<ncell_x; ++x) {
          std::vector<int> tmp = (dim==1)?{x}:((dim==2)?{x,y}:{x,y,z});
    ...
    if (is_mesh_unstructed_) {
      ...
      if (in_fuel.is_open ()) {
        while (std::getline (in, line)) {
          ...
          for (unsigned int x=0; x<ncell_x; ++x) {
            std::vector<int> tmp = (dim==1)?{x}:((dim==2)?{x,y}:{x,y,z});
            relative_position_to_fuel_id_[tmp] = 
                std::atoi (strings[x].c_str()) - 1;
          }
          ct += 1;
        }
        AssertThrow (ct==ncell_y*ncell_z,
...
template <int dim>
void MeshGenerator::GetPinCenter (
    dealii::Triangulation<dim>::cell_iterator &cell,
    dealii::Point<dim> &pin_center) {
...
template <int dim>
void MeshGenerator<dim>::SetPinManifold (
    dealii::Triangulation<dim>::cell_iterator &cell,
    const dealii::Point<dim> pin_center,
    const unsigned int manifold_id) {
  // TODO
}

template <int dim>
void MeshGenerator<dim>::SetManifolds (dealii::Triangulation<dim> &tria,
